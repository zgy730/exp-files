    ...
    async def run(self, dir_path: str = "", file_paths: list[str] = []):
        ...
        try:
            ...
            if isinstance(self.strategy, WordFrequencyStrategy):
                ...
                for i, match in enumerate(result[:10], 1):  # Show first 10 matches
                    print(f"\n{i}. File: {match['file']}")
...
def main(dir, file, strategy, search_query, case_sensitive):
    ...
    Example usage:
        ...
        exp-files --file file1.txt --file file2.txt --strategy sentence_search --search-query "Specific Text" --case-sensitive
    """
    # Create appropriate strategy based on command line option
    if strategy == "sentence_search":
        if not search_query:
            click.echo("Error: --search-query is required for sentence_search strategy", err=True)
